...
from .qwen_layer import QwenLayer, QwenAwqLayer, QwenPackedLayer, QwenPackedOffloadLayer, QwenFBGEMMLayer
...
class Qwen(LLMBase):
    def __init__(self, 
        ...
        self.config.vocab_size = QWEN_2_5_VOCAB_SIZE
...
class QwenAwq(Qwen):
    ...
    def alloc(self, **kwargs):
        ...
        self.cos_cache = self.cos_cache.to(self.dtype)
        self.sin_cache = self.sin_cache.to(self.dtype)
        self.layers :list[QwenAwqLayer] = []
        
        for idx, hf_layer in enumerate(hf_model.model.layers):
            layer = QwenAwqLayer(idx)
            layer.init_parameters(hf_layer=hf_layer)
            layer.to(self.device)
            self.layers.append(layer)
            hf_model.model.layers[idx] = None
...
class QwenFBGEMM(Qwen):
    ...
    def alloc(self, **kwargs):
        ...
        self.cos_cache = self.cos_cache.to(self.dtype)
        self.sin_cache = self.sin_cache.to(self.dtype)
        self.layers :list[QwenFBGEMMLayer] = []
        
        for idx, hf_layer in enumerate(hf_model.model.layers):
            layer = QwenFBGEMMLayer(idx)
            layer.init_parameters(hf_layer=hf_layer)
            layer.to(self.device)
            self.layers.append(layer)
            hf_model.model.layers[idx] = None
...
class QwenAwqOffload(QwenOffload):
    ...
    def alloc(self, **kwargs):
        ...
        self.num_layers = len(self.layers)
        assert self.num_layers % 2 == 0
        self.buffer = [QwenAwqLayer(-1, self.device) for _ in range(2)]
        self.buffer[0].alloc_space(self.layers[0], self.device)
        self.buffer[1].alloc_space(self.layers[0], self.device)
...
class QwenFBGEMMOffload(QwenOffload):
    ...
    def alloc(self, **kwargs):
        ...
        self.num_layers = len(self.layers)
        assert self.num_layers % 2 == 0
        self.buffer = [QwenFBGEMMLayer(-1, self.device) for _ in range(2)]
        self.buffer[0].alloc_space(self.layers[0], self.device)
        self.buffer[1].alloc_space(self.layers[0], self.device)
...
class QwenCudagraph(Qwen):
    ...
    def alloc(self, **kwargs):
        ...
        self.layers :list[QwenPackedLayer] = []
        
        for idx, hf_layer in enumerate(hf_model.model.layers):
            if exit_layer > 0 and idx >= exit_layer:
                break
            layer = QwenPackedLayer(idx)
            layer.init_parameters(hf_layer=hf_layer)
            layer.to(self.device)
            self.layers.append(layer)
            hf_model.model.layers[idx] = None
